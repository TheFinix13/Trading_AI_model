...
Given a Trade or Setup, produces a human-readable narrative of WHY the system
took the trade. Two flavors:

  - Rule-based setups: list confluences in plain English with the actual prices
    each confluence was detected at, plus the source timeframe per confluence.
...
The point: every trade in the journal can be opened up and audited. If the user
can see exactly what triggered each trade, on which timeframe, and what the
candle-confirmation gate said, they can identify which patterns work in practice
vs which ones look good but lose money in the real world."""
...
# Used by the SETUP-TIME explainer (live setups, not journaled trades).
...
class TradeExplanation:
    ...
    why_taken: list[ExplanationLine] = field(default_factory=list)
...
def _format_confluence(tag: str, setup: Setup) -> ExplanationLine:
    """Render one confluence tag with the actual prices from the setup."""
...
def _shap_contributions(model, feature_cols: list[str], features: dict[str, float],
    ...
    N largest by absolute impact. Returns empty list if SHAP is missing."""
    try:
        import shap
    except ImportError:
...
def explain_trade(trade: Trade, ml_model=None, ml_feature_cols: list[str] | None = None) -> TradeExplanation:
    """Same as explain_setup but also includes the trade outcome."""
...
# Plain-English templates keyed by confluence tag, with separate long/short bodies.
...
def explain_journaled_trade(
    ...
    *,
    ...
    confluence_tfs: dict[str, str] | None = None,
    entry_confirmation: dict | None = None,
) -> dict:
    """Produce the rich, structured narrative the dashboard renders.

    `confluence_tfs` maps each confluence tag to the timeframe that detected it
    (e.g., {'zone': 'M15', 'htf_bias_long': 'D1'}). Used to label every confluence
    with its source TF in the narrative — so users can verify each on the right
    chart in TradingView.

    `entry_confirmation` is the candle-close gate evidence the engine logged. When
    present, renders a dedicated 'ENTRY CONFIRMATION' section showing exactly which
    bar acted as the confirmation, what direction it closed, and whether the stop
    was at risk during that bar. When missing, the narrative will explicitly call
    out that no confirmation was required.

    Returns a JSON-friendly dict — the dashboard template just iterates over it."""
    direction = "long" if trade_row.get("direction") == "long" else "short"
    confluence_tfs = confluence_tfs or {}
    ...
    setup_tf = trade_row.get("sig_tf") or "?"

    # 1. Confluences as rich blocks (title + paragraph + concrete numbers + source TF)
    ...
    for tag in confluences:
        ...
        # Source TF: from the lookup, falling back to the setup's TF for native confluences.
        if tag.startswith("htf_"):
            tf_label = confluence_tfs.get(tag, "HTF")  # D1 / H4
        else:
            tf_label = confluence_tfs.get(tag, setup_tf)
        block = {
            "tag": tag,
            "tf": tf_label,
            "title": tpl["title"],
        ...
        if tag == "zone" and features.get("zone_impulse_pips"):
            ...
            block["facts"].append(f"Aligned with trade dir: "
                                   f"{'yes' if features.get('bos_aligned') else 'no'}")
    ...
    # 2. Market context (regime / momentum)
    ...
    # 3. Outcome warning for force-closed trades
    ...
    # 4. Entry confirmation block
    confirmation_lines: list[str] = []
    confirmation_status = "none"  # 'confirmed' | 'not_required' | 'none'
    if entry_confirmation:
        if entry_confirmation.get("required") in ("True", True):
            cb_time = entry_confirmation.get("confirm_bar_time")
            cb_dir = entry_confirmation.get("confirm_candle_dir", "?")
            cb_open = entry_confirmation.get("confirm_bar_open", "?")
            cb_close = entry_confirmation.get("confirm_bar_close", "?")
            stop_violated = entry_confirmation.get("stop_violated_during_confirmation") in ("True", True)
            confirmed = entry_confirmation.get("confirmed") in ("True", True)
            confirmation_status = "confirmed" if confirmed else "not_required"
            confirmation_lines = [
                f"Required: bar after detection must close {'bullish' if direction == 'long' else 'bearish'} "
                f"and not violate the stop.",
                f"Confirmation bar: {_fmt_local(cb_time)}  open={cb_open} → close={cb_close} ({cb_dir} candle).",
            ]
            if stop_violated:
                confirmation_lines.append("⚠ Stop level was touched during the confirmation bar.")
            confirmation_lines.append(
                f"Result: {'CONFIRMED — entry placed on the next bar open.' if confirmed else 'REJECTED — setup skipped.'}"
            )
        else:
            confirmation_status = "not_required"
            confirmation_lines = ["No candle-close confirmation was required for this entry "
                                  "(rules.require_close_confirmation is False)."]
