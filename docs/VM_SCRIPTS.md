...
| **`setup_self_healing.ps1`** *(new 2026-08-04)* | One-shot idempotent self-healing setup: autologon check, MT5 Startup shortcut, 3 watchdog AtLogOn tasks, Windows-Update weekend-only-install + no-auto-reboot policy | `powershell -ExecutionPolicy Bypass -File scripts\setup_self_healing.ps1` (elevated) | Once per VM (re-run safe); pending since Jul 6, executes docs/08 §"Running 24/5" |
| **`verify_self_healing.ps1`** *(new 2026-08-04)* | Read-only PASS/FAIL audit of every self-healing layer (autologon, MT5, tasks, processes, watchdog logs, `.env` healthcheck URLs, WU policy); exit code 0/1 | `powershell -ExecutionPolicy Bypass -File scripts\verify_self_healing.ps1` | After setup + after any reboot test |
