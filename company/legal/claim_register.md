...
| `check_squad_tape_freshness` | `dict` | Bar-ingestion freshness (I017): oldest `last_bar_times` symbol in `state.json`, aged in MARKET seconds (Sat/Sun excluded); warn > 9 h, alarm > 13 h (labels are bar OPENS, healthy band 4–8 h — recalibrated in D133 after the 5 h/9 h originals warned on a fresh tape); na when the squad never ran; corrupt state alarms. Catches a live-but-starved runtime that `runtime_heartbeat` calls healthy. | `watchdog.check_squad_tape_freshness`. | None. |
